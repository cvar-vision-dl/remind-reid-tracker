...
# ---------------------------------------------------------------------------
# FAIR YOLO INIT: GT-free detection of untracked objects
# ---------------------------------------------------------------------------
# The old ``_select_yolo_init_regions`` used GT to decide which objects are
# pending and matched by class name.  The replacement below is GT-agnostic:
# it compares every YOLO detection against all *currently tracked* masks and
# considers a detection "untracked" when its best IoU with any existing track
# is below a threshold.
# ---------------------------------------------------------------------------

def _select_untracked_yolo_detections(
    *,
    yolo_detections: list[YoloInitDetection],
    current_tracker_masks: list[np.ndarray],
    overlap_iou_threshold: float,
) -> tuple[list[dict[str, np.ndarray | str]], list[YoloInitDetection], list[dict[str, Any]]]:
    """Return YOLO detections that do NOT overlap with any existing track.

    Returns
    -------
    regions : list[dict]
        Ready-to-pass regions for ``tracker.initialize`` / ``tracker.add_objects``.
    accepted_detections : list[YoloInitDetection]
        The raw YOLO detections that were accepted (parallel to *regions*).
    log_rows : list[dict]
        One diagnostic row per YOLO detection (accepted or not).
    """
    ...
    for det_idx, det in enumerate(yolo_detections):
        best_track_iou = 0.0
        for track_mask in current_tracker_masks:
            iou = _mask_iou(det.mask, track_mask)
            if iou > best_track_iou:
                best_track_iou = iou

        is_untracked = best_track_iou < float(overlap_iou_threshold)
        log_rows.append(
            {
                "yolo_det_idx": int(det_idx),
                "yolo_class_id": int(det.class_id),
                "yolo_class_name": str(det.class_name),
                "yolo_confidence": float(det.confidence),
                "best_track_iou": float(best_track_iou),
                "accepted_as_new_track": bool(is_untracked),
            }
        )
        if not is_untracked:
            continue
        ...
        regions.append(
            {
                "name": f"yolo_det_{int(det_idx)}",
                "mask": np.asarray(det.mask).astype(np.uint8, copy=False),
            }
        )

    return regions, accepted_detections, log_rows
...
def evaluate_scene_tar_yolo_init(
    *,
    ...
    """Run D4SM tracking with *fair* YOLO-based object discovery.

    Key difference from the old version: GT is loaded and passed to the
    evaluator so that metrics can be computed, but it is **never** used to
    decide when or what to initialize.  YOLO runs on every frame; any
    detection that does not overlap with an existing track (IoU < threshold)
    is added as a new tracked object.
    """
    if torch is None:
        raise RuntimeError("d4sm requires torch, but the current interpreter cannot import it.")
    ...
    print(
        ...
        f"mask_variant={scene_bundle.mask_variant} | init=fair_yolo (GT for metrics only)"
    )
    ...
    # -- Track bookkeeping (no GT dependency) --
    # Maps internal tracker IDs to the YOLO class that seeded them.
    yolo_class_by_tracker_id: dict[int, str] = {}
    next_track_label_counter = 0
    yolo_init_log_rows: list[dict[str, Any]] = []
    ...
    try:
        ...
        for idx, frame_name in enumerate(frame_names):
            ...
            # ---- load GT (for metrics only — NOT used for init decisions) ----
            gt_t0 = perf_counter()
            ...
            # ---- FAIR YOLO init: run YOLO every frame, no GT guidance ----
            yolo_dets = yolo_provider.segment(frame_bgr)

            init_regions, accepted_dets, det_log_rows = _select_untracked_yolo_detections(
                yolo_detections=yolo_dets,
                current_tracker_masks=current_tracker_pred_masks,
                overlap_iou_threshold=float(yolo_init_min_iou),
            )
            for row in det_log_rows:
                item = dict(row)
                item["frame_id"] = int(frame_id)
                item["n_yolo_detections"] = int(len(yolo_dets))
                item["n_existing_tracks"] = int(len(current_tracker_pred_ids))
                yolo_init_log_rows.append(item)
            ...
            if init_regions:
                ...
                if tracker_initialized:
                    ...
                    if len(new_tracker_ids) != len(accepted_dets):
                        raise RuntimeError(
                            "d4sm did not return the same number of internal IDs after add_objects."
                        )
                    ...
                    if len(new_tracker_ids) != len(accepted_dets):
                        raise RuntimeError(
                            "d4sm did not return the same number of internal IDs after initialization."
                        )
                ...
                for tracker_id, det in zip(new_tracker_ids, accepted_dets):
                    yolo_class_by_tracker_id[int(tracker_id)] = str(det.class_name)
                    next_track_label_counter += 1
            ...
            # ---- build detections for evaluator ----
            ...
            # pred_info uses YOLO class names (not GT)
            pred_info_by_id = {
                int(tracker_id): {
                    "instance_label": f"d4sm_track_{int(tracker_id):04d}",
                    "class_name": yolo_class_by_tracker_id.get(int(tracker_id), None),
                }
                for tracker_id in yolo_class_by_tracker_id.keys()
            }
            ...
            n_active_tracks = int(len(getattr(tracker, "all_obj_ids", []) or []))
            if processed_frames == 1 or processed_frames % 20 == 0 or processed_frames == len(frame_names):
                print(
                    ...
                    f"yolo_dets={len(yolo_dets)} | new_tracks={len(init_regions)} | "
                    f"active_tracks={n_active_tracks} | "
    ...
    yolo_attempts = int(len([r for r in yolo_init_log_rows if r.get("accepted_as_new_track") is not None]))
    yolo_new_tracks = int(sum(1 for r in yolo_init_log_rows if bool(r.get("accepted_as_new_track", False))))
    timing_summary = {
    ...
    results["yolo_init_log"] = list(yolo_init_log_rows)
    summary = results.setdefault("summary", {})
    summary.update(timing_summary)
    summary.update(
        {
            "init_source": "yolo_fair",
            "init_recovery_enabled": False,
            "gt_used_for_init": False,
            "n_total_tracks_created": int(len(yolo_class_by_tracker_id)),
            ...
            "yolo_total_detections_evaluated": int(yolo_attempts),
            "yolo_new_tracks_accepted": int(yolo_new_tracks),
            "yolo_detections_overlapping_existing": int(yolo_attempts - yolo_new_tracks),
        }
    )
...
def write_scene_outputs_yolo_init(
    *,
    ...
    scene_summary_row.update(
        {
            ...
            "gt_used_for_init": summary.get("gt_used_for_init", None),
            "n_total_tracks_created": summary.get("n_total_tracks_created", None),
            ...
            "yolo_total_detections_evaluated": summary.get("yolo_total_detections_evaluated", None),
            "yolo_new_tracks_accepted": summary.get("yolo_new_tracks_accepted", None),
            "yolo_detections_overlapping_existing": summary.get("yolo_detections_overlapping_existing", None),
        }
    )
    ...
    write_csv(
        temp_scene_dir / "yolo_init_log.csv",
        d4sm_batch.annotate_rows(
            results.get("yolo_init_log", []) or [],
            run_id=run_id,
            scene_id=scene_id,
            scene_name=scene_name,
        ),
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description=(
            ...
            "initialization — FAIR evaluation mode.\n\n"
            "YOLO runs on EVERY frame.  Any detection that does not overlap\n"
            "with an existing tracked mask (IoU < threshold) is added as a\n"
            "new track.  GT is used ONLY for computing evaluation metrics,\n"
            "never for deciding which objects to initialize."
        ),
    ...
    yolo.add_argument("--yolo-init-min-iou", type=float, metavar="F", help="Minimum overlap IoU with existing tracks to suppress a YOLO detection. [env: REMIND_D4SM_YOLO_INIT_MIN_IOU, default: 0.1]")
    return p


def _write_failed_scenes_file(batch_dir: Path, failed_scene_errors: dict[str, str]) -> None:
    """Write (or remove) failed_scenes.txt so it reflects current state on disk."""
    failed_path = batch_dir / "failed_scenes.txt"
    failed_ids = sorted(failed_scene_errors.keys())
    if failed_ids:
        write_text(failed_path, ",".join(failed_ids) + "\n")
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not scene_ids:
        print("[D4SM-YOLO-INIT-FAIR][BATCH] No pending scenes for this run.")
        return

    print("[D4SM-YOLO-INIT-FAIR][BATCH] Config (FAIR mode: GT for metrics only):")
    print(f"[D4SM-YOLO-INIT-FAIR][BATCH] Output dir -> {batch_dir}")
    print(f"[D4SM-YOLO-INIT-FAIR][BATCH] D4SM model size -> {runtime_config['model_size']}")
    print(f"[D4SM-YOLO-INIT-FAIR][BATCH] D4SM checkpoint dir -> {runtime_config['checkpoint_dir']}")
    print(f"[D4SM-YOLO-INIT-FAIR][BATCH] D4SM offload to CPU -> {runtime_config['offload_state_to_cpu']}")
    print(f"[D4SM-YOLO-INIT-FAIR][BATCH] YOLO model -> {yolo_model_path}")
    print(f"[D4SM-YOLO-INIT-FAIR][BATCH] YOLO conf={yolo_conf} iou={yolo_iou} imgsz={yolo_imgsz} device={yolo_device or 'auto'}")
    print(f"[D4SM-YOLO-INIT-FAIR][BATCH] YOLO init overlap threshold -> {yolo_init_min_iou}")
    print("[D4SM-YOLO-INIT-FAIR][BATCH] Loading models once...")
    ...
    print("[D4SM-YOLO-INIT-FAIR][BATCH] Models ready.")
    ...
    try:
        for scene_id in scene_ids:
            ...
            if d4sm_batch.scene_dir_is_complete(final_scene_dir):
                print(f"[D4SM-YOLO-INIT-FAIR][BATCH] Skip completed scene -> {scene_id}")
                continue
            if final_scene_dir.exists():
                ...
                print(f"[D4SM-YOLO-INIT-FAIR][BATCH] Incomplete output moved -> {scene_id}: {backup_dir}")
            ...
            try:
                ...
                print(f"[D4SM-YOLO-INIT-FAIR][BATCH] Scene start -> {scene_id}")
                print(f"[D4SM-YOLO-INIT-FAIR][BATCH] Data tar -> {scene_bundle.data_tar_path}")
                print(f"[D4SM-YOLO-INIT-FAIR][BATCH] Annotations tar -> {scene_bundle.annotations_tar_path}")
                ...
                print(f"[D4SM-YOLO-INIT-FAIR][BATCH] Scene completed -> {scene_id}")
                ...
                # Write immediately so the file is available even if the process is killed later
                _write_failed_scenes_file(batch_dir, failed_scene_errors)
                print(f"[D4SM-YOLO-INIT-FAIR][BATCH][ERROR] Scene failed -> {scene_id}: {exc}")
            finally:
    ...
    write_text(batch_dir / "notes.txt", "D4SM YOLO-init FAIR mode: GT used for metrics only, not for init decisions.\n")

    _write_failed_scenes_file(batch_dir, failed_scene_errors)
    failed_ids = sorted(failed_scene_errors.keys())
    if failed_ids:
        print(f"[D4SM-YOLO-INIT-FAIR][BATCH] {len(failed_ids)} scene(s) failed -> {batch_dir / 'failed_scenes.txt'}")

    print("[D4SM-YOLO-INIT-FAIR][BATCH] Done.")
